...
// 初始化播放器
const initPlayer = async (videoId, startSeconds = 0) => {
  if (!apiReady) {
    try {
...
// 監聽最小化狀態
//
// 這裡原本是用 appendChild 把 iframe 在兩個容器之間搬移，但有兩個問題：
//   1. YT.Player 會把 <div id="..."> 直接「取代」成同 id 的 iframe，
//      所以 container.querySelector('iframe') 永遠找不到東西（容器自己
//      就是那個 iframe），結果 iframe 一直卡在最小化用的 1x1 隱藏框裡，
//      展開後影片區是空的 —— 從播放清單點歌一定會踩到，因為
//      selectVideo() 是 playPlaylist() 之後馬上 minimize()
//   2. 就算找對了，搬移 iframe 會讓它重新載入、尺寸歸零
//
// 改成在目標容器重新建立播放器，並用 playerVars.start 接續原本的秒數。
watch(() => playerStore.isMinimized, async () => {
  await nextTick()

  if (!playerStore.isVisible || !playerStore.currentVideo) return

  const targetId = playerStore.isMinimized
    ? 'floating-youtube-player-minimized'
    : 'floating-youtube-player'
  const target = document.getElementById(targetId)
  if (!target) return

  // 目標容器已經是播放中的 iframe 就不用重建
  if (target.tagName === 'IFRAME') return

  let resumeAt = 0
  try {
    if (ytPlayer && playerReady && typeof ytPlayer.getCurrentTime === 'function') {
      resumeAt = ytPlayer.getCurrentTime() || 0
    }
  } catch (error) {
    console.warn('FloatingPlayer: 取得播放進度失敗', error)
  }

  disposePlayerInstance()

  const videoId = playerStore.currentVideo.video_id ||
    extractVideoId(playerStore.currentVideo.youtube_url)
  if (videoId) {
    await initPlayer(videoId, resumeAt)
  }
})
...
/* YT.Player 會把 <div id="floating-youtube-player"> 取代成同 id 的 iframe，
   但只保留 id、不保留 class，所以上面的 .youtube-container 會失效，
   iframe 只剩 HTML 預設的 320x150（width="100%" 這種屬性對 iframe 無效）。
   這裡直接用 id 與後代選擇器把尺寸補回來。 */
.player-body :deep(iframe) {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
...
.floating-player.expanded.fullscreen .player-body :deep(iframe) {
  position: static;
  height: 100%;
}
